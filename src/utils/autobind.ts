...
function bindClass<T extends Constructor>(constructor: T) {
  ...
  var skipMethod = (methodName: string) => {
  ...
  Object.keys(descriptors).forEach(prop => {
    ...
    var boundDescriptor = bindMethod(proto, prop, descriptors[prop]);
...
function bindMethod(target: object, prop?: string, descriptor?: PropertyDescriptor) {
  ...
  var { value: func, enumerable, configurable } = descriptor;
  var boundFunc = new WeakMap<object, Function>();
