...
mellowtel_text2 = Эта версия включает в себя open-source библиотеку { $link }. Эта библиотека позволяет делиться неиспользуемым интернетом с доверенными лабораториями и стартапами, которые используют его для обучения своих моделей.
...
mellowtel_accept_all_info2 = Mellowtel использует только ваше подключение к интернету. Безопасность и конфиденциальность гарантированы на 100%, а библиотека открытого исходного кода для всех.
...
mellowtel_dialog_footer = Спасибо за то прочли до сюда, команды { $devs }.
